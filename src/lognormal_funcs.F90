...
!    if (N==0 .or. M==0) then
    ! DPG_bug_checks - M and N can be negative since gets called for
    ! increments from evaporation (removal of aerosol from activated mode).
    if (abs(N).lt.aeronumber_small .or. abs(M).lt.aeromass_small) then
    ! Maybe should make thresholds smaller since will be divided by the
    ! timestep in some instances.
      MNtoRm=0.0
    else
